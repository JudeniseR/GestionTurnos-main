...
// Si no existe id_tecnico en sesión, buscarlo desde usuarios
...
if (!$id_tecnico) {
  die("Error: No se encontró técnico asociado a este usuario.");
}
?>
<!DOCTYPE html>
<html lang="es">
...
<title>Gestionar Agenda - Técnico</title>
...
<header class="topbar">
  <nav class="navbar">
    <div class="nav-left">
      <a class="brand" href="principalTecnico.php"><i class="fa-solid fa-microscope"></i> Inicio</a>
      <a class="nav-link" href="agendaTecnico.php">Gestionar agenda</a>
      <a class="nav-link" href="turnosTecnico.php">Gestionar turnos</a>
    </div>
    <div class="nav-right">
      <a class="logout" href="../../Logica/General/cerrarSesion.php">Cerrar sesión</a>
      <div class="user">
        <span class="user-name"><?= htmlspecialchars($displayRight ?: 'TÉCNICO') ?></span>
        <span class="user-avatar"><i class="fa-solid fa-user-nurse"></i></span>
      </div>
    </div>
  </nav>
</header>
...
<main class="wrap">
  <div class="page-head">
    ...
    <button class="btn btn-outline btn-sm" onclick="location.href='principalTecnico.php'"><i class="fa-solid fa-house"></i><span>Inicio</span></button>
  </div>
...
(() => {
  const API_BASE           = 'api';
  const API_ESTADO_MES     = `${API_BASE}/agenda_estado_tecnico.php`;
  const API_FRANJAS_DIA    = `${API_BASE}/franjas_dia_tecnico.php`;
  const API_SLOTS_PRIMARY  = `${API_BASE}/agenda_slots_tecnico.php`;
  const API_BLOQ_DIA       = `${API_BASE}/bloquear_dia_tecnico.php`;
  const API_DESBLOQ_DIA    = `${API_BASE}/desbloquear_dia_tecnico.php`;
  const API_BLOQ_SLOT      = `${API_BASE}/bloquear_slot_tecnico.php`;
  const API_DESBLOQ_SLOT   = `${API_BASE}/desbloquear_slot_tecnico.php`;
  const API_CREAR_FRANJA   = `${API_BASE}/crear_franja_tecnico.php`;
  const API_ELIM_FRANJA    = `${API_BASE}/eliminar_franja_tecnico.php`;
  const API_TURNOS_DIA     = `${API_BASE}/turnos_dia_tecnico.php`;
  ...
  async function loadSlots(fecha){
    ...
    if (!turnos.length){
      ...
      turnos.forEach(t=>{
        ...
        const estudio = t.estudio ? ` · ${t.estudio}` : '';
        i.innerHTML = `<strong>${hora}</strong> <span>#${t.id_turno}</span> <span class="badge badge-gray">${pac}${estudio}</span>`;
