...
$id_tecnico = isset($_GET['id_tecnico']) ? (int)$_GET['id_tecnico']
            : (isset($_POST['id_tecnico']) ? (int)$_POST['id_tecnico']
            : (int)($_SESSION['id_tecnico'] ?? 0));
...
try {
    ...
    $q->bind_param('is', $id_tecnico, $fecha); $q->execute();
    $rs = $q->get_result();
    ...
    if ($cn->query("SHOW TABLES LIKE 'estado'")->num_rows) {
        $sql = "SELECT TIME_FORMAT(t.hora,'%H:%i') hh, t.id_turno, t.id_paciente
                 ...
                 WHERE t.id_tecnico=? AND DATE(t.fecha)=? AND e.nombre_estado<>'cancelado'";
    } else {
        $sql = "SELECT TIME_FORMAT(t.hora,'%H:%i') hh, t.id_turno, t.id_paciente
                  FROM turnos t
                 WHERE t.id_tecnico=? AND DATE(t.fecha)=? AND (t.id_estado IS NULL OR t.id_estado<>4)";
    }
    $q = $cn->prepare($sql);
    $q->bind_param('is', $id_tecnico, $fecha); $q->execute();
    $rs = $q->get_result();
    ...
    // helper para verificar si un slot está dentro de alguna franja
    ...
    // --- Generación de los 48 slots del día ---
    $slots = [];
