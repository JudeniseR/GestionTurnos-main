...
function back_with($qs){ header('Location: abmEstudios.php?' . $qs); exit; }
...
$flashText = [
  ...
  'updated'=>'Estudio modificado con éxito.',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($form === 'create') {
        ...
        if ($nombre === '') {
            back_with('status=error&msg=' . rawurlencode('El nombre del estudio es obligatorio.'));
        }

        // Verificar duplicado
        $s = $conn->prepare("SELECT 1 FROM estudios WHERE nombre=? LIMIT 1");
        $s->bind_param('s', $nombre);
        $s->execute();
        if ($s->get_result()->num_rows > 0) {
            $s->close();
            back_with('status=error&msg=' . rawurlencode('Ya existe un estudio con ese nombre.'));
        }
        $s->close();

        $s = $conn->prepare("INSERT INTO estudios (nombre, requiere_preparacion, instrucciones) VALUES (?, ?, ?)");
        $s->bind_param('sis', $nombre, $requiere_preparacion, $instrucciones);
        $ok = $s->execute();
        $s->close();
    ...
    elseif ($form === 'update') {
        ...
        if (!$id_estudio || $nombre === '') {
            back_with('status=error&msg=' . rawurlencode('Datos incompletos.'));
        }

        // Duplicado
        $s = $conn->prepare("SELECT 1 FROM estudios WHERE nombre=? AND id_estudio<>? LIMIT 1");
        $s->bind_param('si', $nombre, $id_estudio);
        $s->execute();
        if ($s->get_result()->num_rows > 0) {
            $s->close();
            back_with('status=error&msg=' . rawurlencode('Ya existe un estudio con ese nombre.'));
        ...
        $s = $conn->prepare("UPDATE estudios SET nombre=?, requiere_preparacion=?, instrucciones=? WHERE id_estudio=?");
        $s->bind_param('sisi', $nombre, $requiere_preparacion, $instrucciones, $id_estudio);
        $ok = $s->execute();
        $s->close();
    ...
    elseif ($form === 'delete') {
        $id_estudio = (int)($_POST['id_estudio'] ?? 0);
        if (!$id_estudio) back_with('status=error');

        $s = $conn->prepare("DELETE FROM estudios WHERE id_estudio=?");
        $s->bind_param('i', $id_estudio);
        $ok = $s->execute();
        $s->close();
...
$edit = null;
if ($action === 'edit' && $id > 0) {
    $s = $conn->prepare("SELECT * FROM estudios WHERE id_estudio=? LIMIT 1");
    $s->bind_param('i', $id);
    $s->execute();
...
if ($action === 'list') {
    if ($search !== '') {
        $like = '%' . $search . '%';
        $s = $conn->prepare("SELECT * FROM estudios WHERE nombre LIKE ? ORDER BY nombre");
        $s->bind_param('s', $like);
    } else {
        $s = $conn->prepare("SELECT * FROM estudios ORDER BY nombre");
    }
...
<main class="container">
  ...
  <div class="card" style="display:flex;align-items:center;justify-content:space-between;gap:12px">
    <?php if ($action==='list'): ?>
      <form method="get" action="abmEstudios.php" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
        <input type="hidden" name="action" value="list"/>
        <input type="text" id="buscador" name="q" value="<?= esc($search) ?>" placeholder="Buscar estudio..." style="min-width:280px"/>
        <a class="btn btn-sm" href="abmEstudios.php?action=new"><i class="fa fa-plus"></i> Nuevo estudio</a>
      </form>
      <a class="btn-outline btn-sm" href="principalAdmi.php"><i class="fa fa-arrow-left"></i> Volver</a>
    <?php else: ?>
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        <a class="btn btn-sm" href="abmEstudios.php"><i class="fa fa-list"></i> Volver al listado</a>
      ...
      <table class="table">
        <thead>
          <tr>
            ...
            <th>Requiere preparación</th>
            <th style="width:180px">Acciones</th>
          </tr>
        </thead>
        <tbody>
          <?php if (empty($rows)): ?>
            <tr><td colspan="4" style="color:#666">No hay estudios cargados.</td></tr>
          <?php else: foreach($rows as $e): ?>
            <tr>
              <td><?= (int)$e['id_estudio'] ?></td>
              <td><?= esc($e['nombre']) ?></td>
              <td><?= (int)$e['requiere_preparacion'] ? 'Sí' : 'No' ?></td>
              <td>
                <a class="btn-outline btn-sm" href="abmEstudios.php?action=edit&id=<?= (int)$e['id_estudio'] ?>"><i class="fa fa-pen"></i> Modificar</a>
  ...
  <?php elseif ($action==='new' || ($action==='edit' && $edit)): ?>
    <div class="card">
      <h2><i class="fa fa-flask"></i> <?= $action==='new' ? 'Nuevo estudio' : 'Modificar estudio' ?></h2>
      <form method="post" autocomplete="off">
        <input type="hidden" name="form_action" value="<?= $action==='new' ? 'create' : 'update' ?>">
        <?php if($action==='edit'): ?>
          <input type="hidden" name="id_estudio" value="<?= (int)$edit['id_estudio'] ?>">
        <?php endif; ?>

        <div class="form-grid">
          <div>
            <label>Nombre del estudio *</label>
            <input type="text" name="nombre" value="<?= esc($edit['nombre'] ?? '') ?>" required>
          </div>
          <div>
            <label><input type="checkbox" name="requiere_preparacion" value="1" <?= !empty($edit['requiere_preparacion']) ? 'checked' : '' ?> style="width:auto;margin-right:6px"> Requiere preparación</label>
          </div>
          <div style="grid-column: span 2;">
            <label>Instrucciones</label>
            <textarea name="instrucciones" rows="4" placeholder="Indique las instrucciones si el estudio requiere preparación..."><?= esc($edit['instrucciones'] ?? '') ?></textarea>
          </div>
        </div>

        <div class="form-actions">
          <a class="btn-outline btn-sm" href="abmEstudios.php"><i class="fa fa-xmark"></i> Cancelar</a>
...
if (buscador && tbody) {
  buscador.addEventListener('input', async () => {
    ...
    const response = await fetch('buscarEstudios.php?q=' + encodeURIComponent(q));
