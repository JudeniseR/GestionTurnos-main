      ...
      timeoutBusqueda = setTimeout(async () => {
        try {
          ...
          if (!pacientes || pacientes.length === 0) {
            resultadosPacientes.innerHTML = '<div style="padding:8px;color:#6b7280">No se encontraron pacientes</div>';
            return;
          }
          ...
          resultadosPacientes.innerHTML = `
            <div style="position:absolute;top:100%;left:0;right:0;background:white;border:1px solid #e5e7eb;border-radius:8px;box-shadow:0 4px 12px rgba(0,0,0,.1);max-height:200px;overflow-y:auto;z-index:100">
              ${pacientes.map(p => `
                <div class="paciente-resultado" data-id="${p.id_paciente}" data-nombre="${p.nombre_completo}" data-dni="${p.dni}" style="padding:10px;cursor:pointer;border-bottom:1px solid #f1f5f9">
                  <div style="font-weight:600">${p.nombre_completo}</div>
                  <div style="font-size:12px;color:#6b7280">DNI: ${p.dni || 'S/D'}</div>
                </div>
              `).join('')}
            </div>
          `;
...
if (formNuevaOrden) {
  formNuevaOrden.addEventListener('submit', async (e) => {
    ...
    if (!idPacienteInput.value) {
      mostrarToast('Debe seleccionar un paciente', 'error');
      return;
    }
    ...
    // Agrega esta línea (estaba en el código original)
    ...
    try {
      ...
      // Temporal: mostrar la respuesta cruda
      const text = await res.text();
      console.log('Respuesta cruda:', text);  // Revisa en F12 > Console
